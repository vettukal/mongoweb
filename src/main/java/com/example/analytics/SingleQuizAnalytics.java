...
public class SingleQuizAnalytics {
	...
	@RequestMapping(value="/get_stud_stat", method=RequestMethod.GET)
	public String getStudent(Model model,@RequestParam("subject") String subject,@RequestParam("email") String email) {
		// we have to map the percentage of students who have correct or incorrect.
				// we will have to use the mark repository.
				...
				List<QuizMarks> qm = markrepo.findBySubjectAndEmail(subject, email);
				for (QuizMarks qiter : qm) {
					if(qiter.getMarks()==null)
						continue;
	...
	@RequestMapping(value="/showstudentmark", method=RequestMethod.GET)
	public String getStudent(Model model,@RequestParam("subject") String subject) {
		
		ArrayList<String> qidlist = new ArrayList<>();
		List<QuizMarks> qm = markrepo.findAll();
		HashSet<String> hs = new HashSet<>();
		for (QuizMarks qiter : qm) {
			if(qiter.getEmail()==null)
				continue;
			hs.add(qiter.getEmail());
		}
		...
		model.addAttribute("subject",subject);
		return "analytics/studentdisplay";
	}
